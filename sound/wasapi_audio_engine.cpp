...
void wasapi_audio_engine::publish_profile(const eq_profile& profile) noexcept {
    ...
    processing_mode_.store(static_cast<std::uint8_t>(profile.processing_mode), std::memory_order_relaxed);
    for (std::size_t index = 0; index < arbitrary_band_count; ++index) {
        const auto& band = profile.arbitrary_bands[index];
        arbitrary_frequency_bits_[index].store(std::bit_cast<std::uint32_t>(band.frequency_hz), std::memory_order_relaxed);
        arbitrary_gain_bits_[index].store(std::bit_cast<std::uint32_t>(band.gain_db), std::memory_order_relaxed);
        arbitrary_q_bits_[index].store(std::bit_cast<std::uint32_t>(band.q), std::memory_order_relaxed);
        arbitrary_enabled_[index].store(band.enabled, std::memory_order_relaxed);
    }
...
eq_profile wasapi_audio_engine::snapshot_profile(std::uint64_t& revision) const noexcept {
    ...
    do {
        ...
        const auto mode = processing_mode_.load(std::memory_order_relaxed);
        profile.processing_mode = mode == static_cast<std::uint8_t>(eq_processing_mode::arbitrary)
            ? eq_processing_mode::arbitrary
            : eq_processing_mode::graphic;
        for (std::size_t index = 0; index < arbitrary_band_count; ++index) {
            auto& band = profile.arbitrary_bands[index];
            band.shape = filter_shape::peaking;
            band.frequency_hz = std::bit_cast<float>(arbitrary_frequency_bits_[index].load(std::memory_order_relaxed));
            band.gain_db = std::bit_cast<float>(arbitrary_gain_bits_[index].load(std::memory_order_relaxed));
            band.q = std::bit_cast<float>(arbitrary_q_bits_[index].load(std::memory_order_relaxed));
            band.enabled = arbitrary_enabled_[index].load(std::memory_order_relaxed);
        }
