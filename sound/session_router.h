...
struct routing_candidate {
    ...
    // The current WASAPI stream endpoint. This is intentionally separate from
    // the saved preference, because an already-open stream does not necessarily
    // move when Windows receives a new app route.
    bool routed_to_cable{};
    bool saved_route_to_cable{};
};
